...
| **running SHARC code** (the firmware's, or our own) | digikit's `work/sharc-emulator` (`6f812e9`), in its own worktree: `tools/sharc_run.py` (`Runner`, any `LoadedMemory`, DN2 1.11 as-is). `scripts/sharc_wavefinder_render.py` shows the pattern: splice blocks into section 7 and run. `docs/wavefinder-feasibility.md`, Milestone 1 |
