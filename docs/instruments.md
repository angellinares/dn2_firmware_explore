...
| **running SHARC code** (the firmware's, or our own) | digikit's `work/sharc-emulator` (`6f812e9`), in its own worktree: `tools/sharc_run.py` (`Runner`, any `LoadedMemory`, DN2 1.11 as-is). `scripts/sharc_waverider_render.py` shows the pattern: splice blocks into section 7 and run. `docs/waverider-feasibility.md`, Milestone 1. **The DN2 engine and voice path** (init, slot dispatch, a machine render) need `scripts/sharc_dn2_fixups.py` for seven runner gaps; `scripts/sharc_waverider_voice.py` caches a post-init snapshot under `out/waverider/`. Milestone 2 |
