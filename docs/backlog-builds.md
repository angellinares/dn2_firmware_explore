...
| — | **LFO4** (the project's named goal) | **DELIVERED** — `lfo4-tick6a_DN2_1.11.syx` | **PASSED on hardware 2026-09-17** — owner: *"works"*. Next: parameters from storage and the UI (v4 page view) instead of the image. **Steps 0, 1, 2 and 3 of that are built and verified under the emulator** (2026-09-19, 2026-09-20): C compiled for the ColdFire and loaded at boot, and LFO4's eight values per sound in a table carried through `memcpy` and `memset`, kept across save and load, and a **per-track** LFO4 in both evaluators. **`lfo4-tick7_DN2_1.11.syx` is waiting for a flash**: tracks 1 and 2 should sweep the filter at different rates and the other fourteen not at all |
