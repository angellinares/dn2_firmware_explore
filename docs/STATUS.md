...
| **Does the fourth LFO tick and apply?** | **BUILT, AWAITING HARDWARE — v6a.** `lfo4-tick6a_DN2_1.11.syx`: four LFOs in both evaluators, the three 1,920-byte state arrays relocated and grown to 2,560 in the unclaimed 25 MB, and LFO4's eight parameters **hard-coded in the image** with `DEST` = Filter BASE and near-full depth. 26 asserted edits, seven stubs, HMAC reproduced. Deliberately has no UI and saves nothing — it isolates the engine question. `docs/lfo4-build-plan.md` §5l |
