...
| **LFO4** | **UNBLOCKED** | **No DSP blocker.** Modulation is generated and applied on the ColdFire; LFO parameters are never sent. **The goal is a real fourth LFO** (owner, 2026-09-15) — a fourth `[MOD]` page, saved with the sound. A **seventh modulation source** is a documented fallback only, and costs per-preset storage. **The tick is found, 2026-09-15** — `0x40137726` generates *and* applies all three LFOs, 16 tracks x 3, with a literal `moveq #2` loop start; LFO4's remaining structural question is where its eight parameters live, because the tick reads LFO *n* at indices `8n+1..8n+8` and a fourth would land on 25-32 | `docs/engine-state.md`, `docs/modulation-matrix.md`, `docs/lfo4-slot-plan.md` |
...
| Page id — range test `(page - 0x1a) <= 2` | **PRICED 2026-09-15, no longer simply blocked.** The test is replicated **six** times, and the bound is a literal `2` in all six. Renumbering — LFO4 at `0x1d`, Retrig to `0x1f` — is **six one-byte edits and 23 data longwords, no cave**; an explicit disjunct instead would be six caves. **But** the scan that says nothing else names Retrig returned four hits and all four were unrelated arithmetic, so that negative is near worthless. **THREE READINGS, 2026-09-15. Route paused.** v1 moved page `0x1d`→`0x1f`: pages drew but read wrong, edits dead. v2 added the routing byte at `0x400dc71e`: **no observable change**. DNX then read pattern A1 back off the device and confirmed the model **eight for eight** — a moved TRIG record writes the **sound** value array at index `record+0x04`, and it is **stored** as a lock under `4·slot+lfo`. `PROB`→lock 14 (LFO2 `DEST`, coarse 58) is the stray modulation. ~~cost is seven one-byte edits~~ and ~~the damage is runtime-only~~ are both **RETRACTED**: a **runtime classifier** consults the page id, is authoritative over `param_set_tables_build`, and is **not found**. **Patterns edited under either probe build carry stray LFO locks** — A1 trk 1 trig 5 has eight. `docs/lfo4-feasibility.md`, "The A1 read settles it" |
| Fourth page-view + `[MOD]` navigation | **Not started** |
| **The LFO tick** | **FOUND 2026-09-15.** `0x40137726`, once per frame from the ISR, all 16 tracks. Reads `SPD MULT FADE DEST WAVE SPH MODE DEP` per LFO; `MULT` 0-11 tempo-synced, 12-23 free at a fixed 14400; phase wraps at 1,382,400,000; waves 0-5 via `0x4020b340` (TRI SIN SQR SAW EXP RMP), `RND` sample-and-hold from a Fibonacci generator at `0x4013739c`; applies `value[DEST] += sample * DEP * fade`, clamped. State 16 x 3 x 40 B. **LFO4 cost from the code:** loop start 2->3, offsets +40/+16, stride 120->160, state arrays 1,920->2,560 B, and a home for 8 parameters. A second single-track tick at `0x401373dc` needs the same | `docs/modulation-matrix.md`, "THE LFO TICK" |
...
| Runtime slot space — 8 contiguous slots | **DECIDED 2026-09-15: extension arrays, not stride growth.** The owner chose *saved per preset* (route b), and the constant survey proved widening is infeasible — `#2388`/`#202`/`#359` are baked into 16–29 functions each, `#100`/`#101` into 100+. Route (b) = extension arrays keyed by the stock object/track index + two serialize hooks (`0x400dde44` deserialize, `0x400ddc52` serialize) routing LFO4 through the stored format's reserved 4th-LFO slot. `docs/lfo4-build-plan.md` |
...
| ~~Where are LFO1–3 advanced and applied?~~ | **ANSWERED 2026-09-15** — `0x40137726`, on the ColdFire, in the audio frame, generator and apply in one function. Found by following the owner's lead that the tick is tied to tempo, after digikit's `ColdfireEMAC` language exposed the real ISR and its 73 callees | `docs/modulation-matrix.md`, "THE LFO TICK". Moves to Closed |
...
| **P-lock the arpeggiator settings** | Per-trig arp mode, range, length and step pattern. Owner's ask, 2026-09-15 | **Scoped, not started.** The arp step `0x4002a0bc` reads its settings from an object at `+351..+358`, not from the lockable 1..99 parameter array, so no lock can name them -- the same wall as the modulators. Three routes; route (a) competes with the modulator p-locks for the same spare indices 100-127. Waiting on DNX for where arp settings are stored | `docs/ideas-backlog.md` §13 |
...
**#74 open** — `analysis/lfo-tick`, carrying everything after #73 merged: the ColdfireEMAC finding, the corrected ISR and module read, the forward/inverse maps, the shared skill with DNX, the LFO tick on both images, and the arp p-lock scoping. #73 merged 2026-09-15. External: `m-dwyer/digikit` **#14** open (LFOs and the modulation matrix are ColdFire code, addresses on DT2 1.16 and DN2 1.11), **#12** and **#11** open.
