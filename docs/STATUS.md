...
| **Can an LFO reach an FX or Master parameter?** | **Costed 2026-09-22, and the blocker moved.** Not the mask (flipping Chorus and Master to `0x1e00` was flashed 2026-09-12 and changed nothing) and not only the enumeration. `DEST` stores `record+12`, a slot number with no identity, and the mirror is **seventeen** 101-slot blocks: block 16 carries the FX and Master values at slots 25–69, confirmed by **five** independent readings of ColdFire code — the fifth being stock's own FX fade-out at `0x40027468`, which writes Reverb Mix, Reverb Decay, Delay Feedback and Delay Mix straight into block 16. An LFO cannot address it because its base is the track's block | **Answered on the instrument, 2026-09-22: it sweeps.** `fxblock16_DN2_1.11.syx` swept Delay Feedback Gain (`B + 3336`) from inside the audio ISR and the delay audibly surged and collapsed — so the DSP **does** read block 16, no other sender wins, and nothing overwrites it later in the frame. §4c's mirror formula is confirmed by construction. **Route A is unblocked**: a new `DEST` code range applied where the frame builder writes block 16, about four or five small patches. `docs/fx-master-modulation.md` §9 |
