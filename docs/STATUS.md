...
| **Can an LFO reach an FX or Master parameter?** | **YES — PROVEN ON HARDWARE 2026-09-23.** `fxdest` was flashed and the owner reported the delay surging and collapsing **and responding to `DEP` and `SPD`** — steering is the evidence, because a hard-coded ramp cannot be steered, so what reaches the FX cell came through the LFO evaluator. The engine half of backlog §4 is delivered. **The browser half is now built and gated too — `fxbrowser_DN2_1.11.syx`, §15** — so the 24 Chorus/Delay/Reverb parameters can be *chosen* from the `DEST` list; it carries `fxdest`'s two edits, so it replaces rather than accompanies it, and it is **not flashed**. **§11's flagged unknown is closed and the answer is benign**: the 26-entry ordering table at `0x4028bfc4` is a list of group ids in display order and it *already contains* 16, 17 and 18 at ranks 11-13, so the FX destinations sort between group 15 and group 19 and the feared hang cannot arise. `docs/fx-master-modulation.md` §14, §15. Earlier, 2026-09-22: `fxblock16` answered the prerequisite on the instrument — a store into `mirror[16]` at `B = 0x800068e4` **sweeps**, so the DSP reads block 16, no other sender wins, and nothing overwrites it later in the frame (§9). Route A's sites were then read and measured: the evaluator's destination write is a read-modify-write whose cell is `%a0 + 2*DEST`, evaluator B cannot see codes above 100 (its own `moveq #100` plus a second bound at `0x401376c2` stopping it at 8), and **item 3 — the destination list at `0x4003951e` — is a matched pair, not a cave**: the list holds *entry numbers* and is fed only by the set's `+0x50` virtual, while the browser's confirm path at `0x40107b0e` converts back with no `lsl.l #8`, so three `DEST` sites need the +76, not two | **Flashed and passed. `fxdest_DN2_1.11.syx`** — one byte (`moveq #100` -> `#127` at `0x40137a8e`) and one cave (`0x40137a9e` -> `0x4028ea02`) make `DEST` codes 101..127 write `mirror[16][code-76]`, with the depth, clamp and store all stock. It carries a hard-coded demonstration — track 1, LFO1, `DEST` = none -> code 111, Delay Feedback Gain — because nothing can *choose* an FX code until item 3 is built. **All four gates pass**: `check_coldfire` 1,539 vs the 1,540 baseline with 0 new; `dnfw inspect` 21/21 + HMAC; `emu_boot_check` booted and drew its UI from reset; and `emu_fxdest` puts every code on the cell route A names -- 101 on `0x800075d8`, 111 on `0x800075ec` (the cell the instrument swept), 124 on `0x80007606` -- while the stock control leaves `%a0` at zero above code 100 and writes nothing. Its first two runs measured nothing and said so rather than reporting a verdict; §13 keeps them. **Flashed 2026-09-23 with the owner's go-ahead; it works.** Superseded by **`fxbrowser_DN2_1.11.syx`** (§15), which adds the browser half and drops `fxdest`'s hard-coded demonstration: one hook on `0x400dc02a` answers slots 101..124 from the FX set's own table, the enumeration bound goes 101 -> 125, three `jsr 0x400dbcc4` sites are repointed at a cave helper that adds 76 for groups 16-18, and ten records' `+44` is opened (Chorus's eight; Delay and Reverb already carried `0x1e00`). 144 bytes. **All four gates pass**, including a snapshot probe that calls the lookup over slots 0..130 and the conversion over entries 1..330 against a stock control: slots 0..100 unchanged, codes 101..124 resolve to the FX entries stock refuses entirely, and the round trip closes for all 24. **The browser itself cannot be gated here — no panel, no page view, no encoder — so the list is the instrument's question.** **FLASHED 2026-09-23 and half-works: the list grew, the encoder cannot reach the new part.** The owner gets to `OVR Routing` and one more click returns him to the top of the list, though the scroll mark shows more below. **Measured with `emu_destlist.py`, which calls the list builder directly: the enumeration is correct** — +24 and -0 for all three `want` masks (`0x200` 76->100, `0x1e00` 55->79, `0x0600` 69->93), with the FX entries immediately after entry 305 `OVR Routing` exactly as designed. So item 3 works and the fault is in what the browser **stores** when one is picked: `0x40107b0e` is a normaliser, not the store, and the value is produced inside `%a2@(32)`, a virtual reached through a function pointer at `this+404` that has not been resolved. Suspected: one of the **31 `jsr 0x400dbcc4` sites left untouched**, so picking Chorus Depth stores 25 instead of 101 and the redraw's search fails, landing on the first element. **FOUND, and fixed in `fxbrowser2_DN2_1.11.syx` (built, gated, not flashed).** digikit's `rttiscan.py` named the class in seconds: `0x40039904` is `ParameterSet`'s vtable `+0x24`, one of only two slots in 1,911 vtables reaching `0x400dbcc4`, and it is unmistakably on the `DEST` path — it `btst`s the record's `+44`, shifts the stored value right by 8 to recover the code and resolves it through `+0x50`. **The root cause is a counting error in §10**: it found the `entry -> slot << 8` sites by scanning for an **adjacent** `lsl.l #8` and reported two; widening the window to 16 bytes finds **six**, four of which put a call between the conversion and the shift. `fxbrowser2` repoints all four missed sites at the same helper (four 4-byte rewrites, 156 bytes total). **§16's "did we break it or expose it" answer is RETRACTED** — it reasoned from `want 0x1e00` when the browser passes `0x200`; stock reaches 21 entries past `OVR Routing` into the Mod destinations, so the build broke those too and changed end-of-list from "stop" to "wrap". `docs/fx-master-modulation.md` §11, §12, §15, §16, §17, §18, §19 |
