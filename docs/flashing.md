...
| 2026-09-12 | **Mod-mask test** (`modmask-test_DN2_1.11.syx`) — two 4-byte writes opening Portamento Time and the AMP envelope Delay Time to modulation | **Pass, and the hypothesis confirmed.** `PORT Portamento Time` now appears in MOD1's destination list on the instrument. See `docs/modulation-mask.md`. |
