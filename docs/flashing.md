...
| 2026-09-17 | **Boot screen via the mod pipeline** (`bootscreen-bang_DN2_1.11.syx`, base **1.11**) — the bang, built by `dnfw mods apply --mod bootscreen` (`src/dnfw/mods/bootscreen.py`) instead of a one-off script | **PASS.** Owner: *"works"*. The code path the website's boot-screen page will use is proven on the instrument. |
| 2026-09-18 | **Boot screen, ASCII glitch** (`bootscreen-ascii_DN2_1.11.syx`, base **1.11**) — `dnfw mods apply --mod bootscreen --boot-animation ascii`: 112 pre-rendered frames in an `ANIM` chunk (115 KB appended), played over the intro | **PASS.** Owner: *"boot is ok, no issues, indeed more fluent than the GIF"*; *"eveything was correct"* — upright, so the emulator's finding that the plain blit flips the source vertically holds on hardware. Boot not slowed by the larger copy. |
| 2026-09-18 | **Boot screen, spin (1966)** (`bootscreen-spin_DN2_1.11.syx`, base **1.11**) — `--boot-animation spin`: 168 frames (172 KB appended) through the same `ANIM` path | **PASS.** Same report, both builds flashed. |
| 2026-09-17 | **Static burst intro** (`intro-burst_DN2_1.11.syx`, base **1.11**) — a white burst with the logo knocked out, written over the intro source bitmap every frame from an appended mask/value table | **UNCLEAR — no explosion seen.** Owner: *"cannot see any explosion on this one..."*. Under the emulator the same build holds still for frames ~12–73 and is scattered by the tunnel from ~87 (`docs/img/intro-burst-film.png`). Follow-up, owner: *"the animation looks like an angled screen cutting off the logo with the bang"* — so the burst **does** show and the intro's **closing angled sweep** cuts it off; the scatter phase between them was not perceived. Emulator frames +24M–+56M show that scatter, +60M the angled sweep; on the instrument the scatter is either too brief to read or the white burst's fragments read as noise. **Boots and works; the explosion is the unclear part.** Superseded by `intro-bang` (passed), which uses the same hooks with whole images. |
| 2026-09-17 | **NOIS loop length, v5** (`lfo-waveshapes5_DN2_1.11.syx`, base **1.11**) — per-LFO cycle counter; SPH = `colour.loop`, the display through the `Start Phase` records' formatter reading WAVE via an active-track byte and the engine mirror | **PARTIAL.** Owner: SPH still shows a plain 0–127 number, so the colour and loop cannot be dialled by eye; *"the sound changes with it and is nice but I cannot assert"*; glyph still RND. Cause: the formatter found WAVE through the active-track byte `0x42431a6c`, read under the emulator — but that byte lives in a struct reached through a heap pointer (`ProjectSettings`+16), so its address is not fixed on the instrument; the check failed and fell through to the stock number. v6 asks the ParameterSet for WAVE instead. |
| 2026-09-17 | **NOIS loop length, v6** (`lfo-waveshapes6_DN2_1.11.syx`, base **1.11**) — v5 with the SPH display moved to a wrapper of the sound ParameterSet's format method (vtable `0x401db7fc` slot 92), asking the object for WAVE | **PASS.** Owner: *"it works!"* SPH shows `colour.loop` on NOIS and the loop length controls repetition. The current LFO waveshapes build. Remaining: MOD-page glyph still RND. |
| 2026-09-17 | **Glyphs and labels, v8** (`lfo-waveshapes8_DN2_1.11.syx`, `lfo-wavetable4_DN2_1.11.syx`, base **1.11**) — MOD-page glyph rendered from each new generator at draw time; SPH renamed STPS/WDTH/TYPE/RPTS via `getShortName` | **PASS**, one defect. Owner: *"the new waves glyphs work great"*. Defect: **the NOIS glyph changes on unrelated button presses** — every redraw re-renders it. Cause: the renderer calls NOI with reserved key 1023, whose cycle counter advances once per redraw (each render sweeps a whole cycle, so the next one sees a wrap), so the picture shows the next cycle each time. Fix in v9: clear key 1023's state before rendering. **Second report, MIDI tracks:** *"all shapes are usable in the MIDI LFOs tracks but the SPH number doesn't change as required and the name of that knob function also doesn't change"*. SPH number: the MIDI LFO page formats through `MidiParameterSet` (vtable `0x401db994`), whose slot 92 was not wrapped — fixed in v10. Label: under the emulator (track forced MIDI by patching the UI mask test `0x40031274`), v9's `getShortName` hook **does** show `TYPE` on the MIDI LFO page with NOIS poked in, so where the label stayed stock on the instrument is not yet reproduced. |
| 2026-09-17 | **Three wavetables** (`lfo-wavetables_DN2_1.11.syx`, base **1.11**) — WTB1 basic shapes, WTB2 harmonic sweep, WTB3 vowels; 7 frames × 32 bytes each, bilinear interpolation, SPH = position (`POS`) | **PASS.** Owner: *"the wavetable works perfectly."* |
| 2026-09-17 | **Every new LFO waveform combined** (`lfo-waves_DN2_1.11.syx`, base **1.11**) — 46-byte boot copy stub in a cave; all code and tables in a 2,672-byte blob appended to MAIN OS and copied to `0x46780000` at startup | **PASS.** Owner: *"It boots. And works. all 3 points."* — boot, all seven waves on audio and MIDI tracks with glyphs and renamed knobs, stable NOIS glyph. **First hardware proof that code (not just data) runs from the appended area copied to RAM.** Observation: TRAP's glyph barely changes from SLOP 0 to ~94 — the gain law `1 + (127−SPH)/4` keeps the edges under two pixels of the 28-pixel tile for most of the range. |
