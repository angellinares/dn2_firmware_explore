...
| **Can an LFO reach an FX or Master parameter?** | **Yes, and the engine half is built. 2026-09-22.** `fxblock16` answered the prerequisite on the instrument — a store into `mirror[16]` at `B = 0x800068e4` **sweeps**, so the DSP reads block 16, no other sender wins, and nothing overwrites it later in the frame (§9). Route A's sites were then read and measured: the evaluator's destination write is a read-modify-write whose cell is `%a0 + 2*DEST`, evaluator B cannot see codes above 100 (its own `moveq #100` plus a second bound at `0x401376c2` stopping it at 8), and **item 3 — the destination list at `0x4003951e` — is a matched pair, not a cave**: the list holds *entry numbers* and is fed only by the set's `+0x50` virtual, while the browser's confirm path at `0x40107b0e` converts back with no `lsl.l #8`, so three `DEST` sites need the +76, not two | **Gated, not flashed. `fxdest_DN2_1.11.syx`** — one byte (`moveq #100` -> `#127` at `0x40137a8e`) and one cave (`0x40137a9e` -> `0x4028ea02`) make `DEST` codes 101..127 write `mirror[16][code-76]`, with the depth, clamp and store all stock. It carries a hard-coded demonstration — track 1, LFO1, `DEST` = none -> code 111, Delay Feedback Gain — because nothing can *choose* an FX code until item 3 is built. **All four gates pass**: `check_coldfire` 1,539 vs the 1,540 baseline with 0 new; `dnfw inspect` 21/21 + HMAC; `emu_boot_check` booted and drew its UI from reset; and `emu_fxdest` puts every code on the cell route A names -- 101 on `0x800075d8`, 111 on `0x800075ec` (the cell the instrument swept), 124 on `0x80007606` -- while the stock control leaves `%a0` at zero above code 100 and writes nothing. Its first two runs measured nothing and said so rather than reporting a verdict; §13 keeps them. **Needs the owner's go-ahead before it goes near the instrument.** `docs/fx-master-modulation.md` §11, §12 |
