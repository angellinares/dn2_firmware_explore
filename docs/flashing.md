...
| 2026-09-17 | **Static burst intro** (`intro-burst_DN2_1.11.syx`, base **1.11**) — a white burst with the logo knocked out, written over the intro source bitmap every frame from an appended mask/value table | **UNCLEAR — no explosion seen.** Owner: *"cannot see any explosion on this one..."*. Under the emulator the same build holds still for frames ~12–73 and is scattered by the tunnel from ~87 (`docs/img/intro-burst-film.png`). Not yet known what the instrument showed instead (static burst throughout? stock tunnel?). Superseded by `intro-bang` (passed), which uses the same hooks with whole images. |
