...
| Page id — range test `(page - 0x1a) <= 2` | **PRICED 2026-09-15, no longer simply blocked.** The test is replicated **six** times, and the bound is a literal `2` in all six. Renumbering — LFO4 at `0x1d`, Retrig to `0x1f` — is **six one-byte edits and 23 data longwords, no cave**; an explicit disjunct instead would be six caves. **But** the scan that says nothing else names Retrig returned four hits and all four were unrelated arithmetic, so that negative is near worthless. **TWO FLASHES, 2026-09-15 — the cost model is RETRACTED.** v1 moved page `0x1d`→`0x1f`: pages drew but read wrong and edits were dead. v2 added the routing byte `moveq #29`→`moveq #31` at `0x400dc71e`: **no observable change at all.** Diagnosis: the moved records fall through to the **sound** parameter set and read/write the sound value array at index `record+0x04` — `VEL` reads **112**, which is LFO1 `SPD`'s default, and p-locking `PROB` (index 12) **re-aims LFO2's `DEST`**. So a **runtime classifier** consults the page id, is authoritative over `param_set_tables_build`, and has **not been found**. ~~seven one-byte edits~~ — do not trust that figure. Next: find who picks between the four `ParameterSet` accessors at `0x400dc02a/0b0/0ca/0e4` — `docs/lfo4-feasibility.md`, "v2, and the diagnosis" |
