...
| Page id — range test `(page - 0x1a) <= 2` | **PRICED 2026-09-15, no longer simply blocked.** The test is replicated **six** times, and the bound is a literal `2` in all six. Renumbering — LFO4 at `0x1d`, Retrig to `0x1f` — is **six one-byte edits and 23 data longwords, no cave**; an explicit disjunct instead would be six caves. **But** the scan that says nothing else names Retrig returned four hits and all four were unrelated arithmetic, so that negative is near worthless. **ASKED THE DEVICE 2026-09-15, and the answer was no — then one byte.** The probe moved page `0x1d` to `0x1f`; the pages still drew but read **zero** and no edit reached the sequencer. Cause found: `param_set_tables_build`'s routing cascade ends in an **exact match** at `0x400dc71e` (`moveq #29` for `0x1d`, `moveq #30` for `0x1e`, everything else falls out with no table). So the page id *is* named elsewhere — in a form no scan would have found. **Renumbering is not dead, it was one byte short.** `page-renumber-test2` adds `moveq #29`→`moveq #31` and is built and verified; **not flashed**. Revised full cost: 22 data words + **7** one-byte code edits + LFO4's ten records, still no cave — `docs/lfo4-feasibility.md`, "What the flash answered" |
