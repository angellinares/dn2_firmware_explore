...
| 8 | New LFO waveforms | **DELIVERED** — ~~`lfo-waveshapes_DN2_1.11.syx`~~ **FAILED on hardware 2026-09-17** (ran as RND, named ERR: WAVE clamped to 6 in both evaluators, formatter bound 6). **v2 `lfo-waveshapes2_DN2_1.11.syx`** | **v2 PASSED on hardware 2026-09-17** — all three waves work; glyph still RND. NOI ignored SPD/MULT (shared state) → v3 `lfo-waveshapes3` **passed**; ~~v4~~ superseded; **v5 `lfo-waveshapes5`** (SPH = `colour.loop`) awaiting a flash |
