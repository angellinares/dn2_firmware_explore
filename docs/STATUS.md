...
| **Is there a minimum-version gate on the DN2?** | **YES — confirmed from our own image 2026-09-16, partially located.** The upgrade error table at `0x40208748` carries **`Unsupported downgrade`** at index 5, consumed by the OS-upgrade state machine `0x40129c56`; the UI has a second string `Downgrade not possible` at `0x4021f3bc`. So lalzart's account and ours now agree. **Not yet found:** which comparison produces the code — the validator `0x400dbc4c` returns only 1/3/4 and never 6 | `docs/version-gate.md` |
