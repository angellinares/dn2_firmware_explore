...
| **`parameter_value_getter`** ⚠️ **MIS-ANCHORED** — forcing its return changes nothing on screen, so it is **not the display path** (2026-09-13). The instruction it was anchored on is real; the *role* the name claims is not. | `0x40064786` | **`0x4006408a`** | the function containing the `+0x4f358` reference at `0x400640dc`; reads an LFO value as `sub + 0x14 + idx*2` |
