...
## 11. The dense block is the compressed factory sound bank
...
**factory preset/sound content, LZ-compressed inside MAIN OS**.
...
ASCII runs finds **591 name-shaped strings**, and they read exactly like an
Elektron factory bank, each truncated where a match reference interrupts the
literal run:
...
**The DN2 ships its factory sounds compressed inside MAIN OS.** That was not
previously recorded anywhere in this repository, and it bears on
...
It also reframes the transient hunt. A *compressed* transient bank would look
