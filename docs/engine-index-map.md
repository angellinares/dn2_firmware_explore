...
| **`blob`** (id 7, 836,956 B) | ~~**Not a 48-bit instruction stream.** Per-byte-position entropy at stride 6 is flat (spread 0.36) where stride 4 shows real column structure (spread 1.58, final byte 5.59) — the signature of little-endian float32, not SHARC's 48-bit words. Consistent with the existing "mixed data, largely float32" reading.~~ **[SUPERSEDED 2026-09-13 — `docs/sharc-image.md`.]** It **is** the SHARC program, as an **ADI boot stream**: 16-byte `{code, target, count, argument}` headers with payloads between them, so there is no global 6-byte period to find even though the payloads are SHARC code. The measurement was right; the inference did not follow. Windowed, the first 128 KB shows stride-4 spread **3.18** against 1.17 for the whole section — the float32 tail is 575 KB of 837 KB and swamps any whole-file average. |
